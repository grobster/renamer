...
public class RenamerViewConfigLms implements FileRenamerObserver {
	...
	public RenamerViewConfigLms(FileRenamerInterface renamer) {
	...
	public void createView() {
		...
		frame = new JFrame(" Simple Photo Consent Repairer 1.0.2");
	...
	private void buildCompanyCombo() {
		Path path = Paths.get("C:\\ProgramData\\FileRenamer\\renamer_config.txt");
	...
	private void createProgramFolder() { //method must run before buildCompanyCombo method in constructor
		Path path = Paths.get("C:\\ProgramData\\FileRenamer\\");
		Path logPath = Paths.get("C:\\ProgramData\\FileRenamer\\logs"); // test
		if (!Files.exists(path)) {
			try {
